...
const STATUS_LABELS = computed(() => ({
  assigned: t("deliveries.status.assigned"),
  done: t("deliveries.status.done"),
}));
...
<template>
  <div>
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-semibold text-brand-brown">{{ t("deliveries.title") }}</h1>
      <Button size="sm" @click="router.push({ name: 'delivery-new' })">
        {{ t("deliveries.newButton") }}
      </Button>
    </div>

    <p v-if="isLoading" class="mt-6 text-sm text-brand-brown/60">{{ t("deliveries.loading") }}</p>
    <p v-else-if="isError" class="mt-6 text-sm text-red-600">{{ t("deliveries.loadError") }}</p>
    ...
    <div v-else class="mt-6 overflow-x-auto rounded-xl border border-brand-brown/10 bg-white">
      <table class="w-full text-left text-sm">
        <thead class="border-b border-brand-brown/10 text-brand-brown/60">
          <tr>
            <th class="px-4 py-3 font-medium">{{ t("deliveries.table.date") }}</th>
            <th class="px-4 py-3 font-medium">{{ t("deliveries.table.courier") }}</th>
            <th class="px-4 py-3 font-medium">{{ t("deliveries.table.status") }}</th>
            <th class="px-4 py-3 font-medium">{{ t("deliveries.table.cashExpected") }}</th>
            <th class="px-4 py-3 font-medium">{{ t("deliveries.table.cashCollected") }}</th>
          </tr>
        </thead>
        <tbody>
          <tr
