...
id · user_id · endpoint UNIQUE · p256dh · auth · created_at. `company_id` yo'q — foydalanuvchiga bog'liq, kompaniyaga emas (bitta foydalanuvchi bir nechta kompaniyaga a'zo bo'lsa ham bitta brauzer obunasi). RLS'siz (izolyatsiya servis qatlamida `user_id = auth.userId`). Faza 6da Web Push uchun qo'shildi.
