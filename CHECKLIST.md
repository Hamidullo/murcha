...
- [x] **Prod konfig qattiqlashtirildi** — prod'da standart sir bilan ishga tushish endi IMKONSIZ: compose `POSTGRES_PASSWORD`/`MINIO_ROOT_PASSWORD`/`GHCR_IMAGE_PREFIX`siz to'xtaydi, API `JWT_ACCESS_SECRET` standart/qisqa bo'lsa to'xtaydi. Uptime Kuma internetdan yopildi (`127.0.0.1:3001`). `VITE_VAPID_PUBLIC_KEY` build-arg orqali (Vite uni build vaqtida qotiradi). `DEPLOY.md` runbook yozildi
- [x] **CI RLS'ni tekshiradi** — Postgres 17 servisi bilan alohida `rls` job'i: migratsiya → RLS → rollar → seed → `db:verify-rls` (15 tasdiq). Ilgari CI'da Postgres umuman yo'q edi, ya'ni RLS butunlay buzilsa ham 990 test yashil qolaverardi. `actionlint` ham qo'shildi — u `deploy.yml`dagi ikki marta o'tkazib yuborilgan `secrets` konteksti xatosini topdi
- [ ] **Real deploy (odam qismi)** — `DEPLOY.md`: server, DNS, sirlar, SSL bootstrap, GitHub secrets/variables, birinchi deploy. Birinchi qaror: prod Postgres qaysi (RDS/Cloud SQL bo'lsa master'ga `BYPASSRLS` kerak, aks holda super-admin/vitrina buziladi)
