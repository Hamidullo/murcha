...
Faza 12 oxirida "deployga tayormi?" tekshiruvi bir nechta bloklovchi kamchilikni ochdi: migratsiya umuman yaratilmagan edi (sxema hech qachon haqiqiy Postgres'ga qo'llanilmagan — 988 test mock repository ustida), API Docker image'i ikki sababdan ishga tushmasdi, `deploy.yml` sintaksis xatosi bilan parse bo'lmasdi, RLS esa mavjud bo'lib turib aslida qo'llanmasdi.

- [x] **Task 1 — Migratsiya + Docker/CI tuzatishlari**: `migrations/20260717090000_init` (`migrate diff` bilan offline generatsiya + `stock.sql`/`checks.sql`/`immutable.sql` qo'lda birlashtirildi); `stock.sql` `DROP CONSTRAINT` → `DROP INDEX` (Prisma `@@unique`ni indeks qiladi — eski kod xato berardi); `apps/api/Dockerfile`ga `prisma generate` + `packages` manbasini kontekstdan ko'chirish (ikkalasi ham ishga tushishda crash berardi); `docker-compose.prod.yml`ga bir martalik `migrate` xizmati (api/worker `service_completed_successfully` kutadi — poyga yo'q); `deploy.yml`da `secrets` sharti qadam darajasiga (job darajasidagi `if`da `secrets` konteksti mavjud emas — butun workflow parse bo'lmasdi)
- [x] **Task 2 — RLS'ni haqiqatan yoqish**: `prisma/roles.sql`+`setup-roles.js` (`murcha_app` — LOGIN/NOBYPASSRLS/ega emas), `rls.sql`ga `FORCE ROW LEVEL SECURITY` va aniq `WITH CHECK` (ikki huquq eskalatsiyasi yopildi: `company_members`ga o'zini istalgan kompaniyaga qo'shish, `roles`/`units`ga soxta "tizim" qatori yozish); `lib/prisma.js` ikki client (`prisma`/`prismaBypass`, prod'da `DATABASE_ADMIN_URL` majburiy); `withBypass()` qo'shildi va `platform`/`showcase` shunga o'tkazildi (vitrinada bypass qamrovi bitta slug so'roviga toraytirildi); `schema.prisma` `directUrl`. 990/990 test, lint 0 xato
- [x] **Task 3 — Haqiqiy Postgres'da tekshirish**: migratsiya birinchi marta Postgres 17.10'ga qo'llandi (49 jadval, 19 CHECK, 5 immutable trigger, `stock` indeksi `NULLS NOT DISTINCT: true`), `pnpm db:verify-rls` 10/10, API image qurildi va `/healthz` → `{"db":true,"redis":true}`, uchidan-uchiga API sinovi (ro'yxatdan o'tish → login → kompaniyalararo izolyatsiya → IDOR 404 → platform `withBypass` ikkala kompaniyani ko'radi → vitrina 200/404). Sinov faqat kod yozib bo'lmaydigan **4 ta haqiqiy bug**ni ochdi (quyida)

  Faqat haqiqiy DB'da chiqqan xatolar:
  1. **`''::uuid` (22P02)** — `set_config(..., true)` GUC'ni sessiyada "mavjud" qiladi, tranzaksiyadan keyin qiymat NULL'ga emas BO'SH SATRGA qaytadi. Prisma pool'i tufayli prod'da: tenant so'rovi ulanishni iflos qiladi → o'sha ulanishdagi keyingi login yiqiladi. Yechim: `NULLIF(current_setting(...), '')::uuid`
  2. **Login RLS bilan umuman ishlamasdi** — `findByUserId()` `include: { company: true, role: true }` qiladi; `company_members`da a'zolik istisnosi bor edi, `companies`/`roles`da yo'q → bog'liq qator `null` → Prisma "Field company is required to return data". Yechim: ikkalasiga a'zolik filiali (`EXISTS` + `status='active'`), `WITH CHECK` tor qoldi
  3. **`.dockerignore` umuman yo'q edi** — `COPY apps/api ./apps/api` host'ning `node_modules`ini ham ko'chirib, `deps` bosqichidagi symlink'lar ustiga yozardi (Windows link'lari Linux'da ishlamaydi) → `Cannot find module .../prisma`. Barcha image'larga taalluqli edi
  4. **`ERR_PNPM_WORKSPACE_PKG_NOT_FOUND`** — `runner` bosqichiga `pnpm-workspace.yaml`/root `package.json` ko'chirilmagan; `packages/shared/node_modules` (zod) ham yetishmasdi
